...
def main():
    p = argparse.ArgumentParser(
        description="Check dataset for contractions (words with apostrophes)"
    )
    p.add_argument("input_csv", help="Path to input CSV file")
    p.add_argument(
        "--text-col", default="lyrics", help="Name of the text column in CSV"
    )
    p.add_argument(
        "--top", type=int, default=50, help="Show top N most frequent contractions"
    )
    args = p.parse_args()
    ...
    if args.text_col not in df.columns:
        raise SystemExit(
            f"Column '{args.text_col}' not found. Available: {list(df.columns)}"
        )
    ...
    print(
        f"Found {len(counter)} unique contractions | {sum(counter.values())} total occurrences\n"
    )
