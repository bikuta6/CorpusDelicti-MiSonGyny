...
def print_structure(module, name="model", max_depth=2):
    def _rec(m, prefix, depth):
        ...
        print(f"{prefix}{name if prefix=='' else ''}{('' if prefix=='' else '')}{''} - {m.__class__.__name__} | params={total} trainable={trainable}")
        if depth >= max_depth:
            return
...
def try_load_model(model_id):
    ...
    try:
        cfg = AutoConfig.from_pretrained(model_id, output_hidden_states=False, output_attentions=False)
        try:
            model = AutoModelForSequenceClassification.from_pretrained(model_id, config=cfg, low_cpu_mem_usage=True)
        except Exception:
            model = AutoModel.from_pretrained(model_id, config=cfg, low_cpu_mem_usage=True)
