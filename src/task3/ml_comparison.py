...
if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser(
        description="ML Baseline Comparison for Task 3"
    )
    ...
    print(f"Loading data for Task 3 with baseline={args.baseline}...")
    base = os.path.join(os.path.dirname(__file__), "..", "..")
    train_df = pd.read_csv(os.path.join(base, "data", "task3", f"{pre}train_df.csv"))
    val_df = pd.read_csv(os.path.join(base, "data", "task3", f"{pre}val_df.csv"))
    test_df = pd.read_csv(os.path.join(base, "data", "task3", f"{pre}dev_df.csv"))
    ...
    results_file = os.path.join(
        base, "results", "task3", f"ml_{is_baseline_str}_results.csv"
    )
