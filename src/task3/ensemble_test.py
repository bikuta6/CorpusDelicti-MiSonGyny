...
MODELS_BASE_DIR = "../../models/task3/comparison"
TEST_PATH = "../../data/task3/processed_test.csv"
RESULTS_FILE = "../../results/task3/submission_ensemble.csv"
BATCH_SIZE = 32
...
def main():
    ...
    predictions = ["Y" if p >= final_threshold else "N" for p in avg_probs]
    ...
    true_labels_df = pd.read_csv("../../data/task3/test_labels.csv")
    y_true = true_labels_df["label"].map({"N": 0, "Y": 1}).values
    y_pred = np.array([1 if p == "Y" else 0 for p in predictions])
