...
def infer_label_type(series: pd.Series) -> str:
    ...
    Infer label type from series values:
      ...
      - 'binary_text' for {'N','Y'} or any 2 unique strings
...
def compute_positive_mask(labels: pd.Series) -> tuple[np.ndarray | None, str]:
    ...
    if lt == "binary_text":
        # Prefer common misogyny setup labels: N/Y
        normalized = vals.astype(str).str.strip()
        uniq = set(normalized.unique())
        if "Y" in uniq and "N" in uniq:
            return (normalized == "Y").to_numpy(), "Y"
...
def main():
    ...
    parser.add_argument(
        "--csv-path",
        type=str,
        default="data/task3/processed_train.csv",
